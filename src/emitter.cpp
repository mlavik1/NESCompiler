...
uint16_t Emitter::Emit(const char* op, EAddressingMode addrMode, uint8_t val)
{
    return Emit(op, addrMode, (uint16_t)val);
}
...
uint16_t Emitter::Emit(const char* op, EAddressingMode addrMode, uint16_t val)
{
    ...
    LOG_INFO() << opcode.mName << " " << val;
    
    uint16_t operandLen = 0;
    if (addrMode == EAddressingMode::Implied || addrMode == EAddressingMode::Accumulator)
        operandLen = 1;
    else if (addrMode == EAddressingMode::Absolute || addrMode == EAddressingMode::AbsoluteX || addrMode == EAddressingMode::AbsoluteY)
        operandLen = 2;
